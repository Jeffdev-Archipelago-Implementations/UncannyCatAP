...
var open_save: APSaveFile
var open_save_ind := -1

func make_save_file() -> APSaveFile: ## Override to return a subclass of APSaveFile
	return APSaveFile.new()
...
func _ready() -> void:
	if OS.is_debug_build(): # Debug commands for forcibly saving/loading via console
		AP.inst.cmd_manager.register_command(ConsoleCommand.new("/save").debug()
		...
		AP.inst.cmd_manager.register_command(ConsoleCommand.new("/delsave").debug()
		...
		AP.inst.cmd_manager.register_command(ConsoleCommand.new("/loadsave").debug()
			...
			.set_call(func(mgr: CommandManager, cmd: ConsoleCommand, msg: String):
				...
				if command_args.size() != 2 or not command_args[1].is_valid_int() or int(command_args[1]) < 0:
					...
					var is_conn: bool = AP.inst.status != AP.APStatus.DISCONNECTED
					if is_conn:
						AP.inst.ap_disconnect()
						while AP.inst.status != AP.APStatus.DISCONNECTED:
							await AP.inst.status_updated
					read_save(int(command_args[1]))
					if is_conn:
						AP.inst.ap_reconnect_to_save()))
...
func read_save(ind: int) -> bool:
	...
	if not file or file.get_pascal_string() != SAVE_HEADER:
		...
		file.close()
	open_save_ind = ind
	AP.inst.creds = open_save.creds
	AP.inst.aplock = open_save.aplock
	return true
