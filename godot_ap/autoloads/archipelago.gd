...
static func _log(s: String) -> void:
	...
	print(msg)
	if AP.inst:
		AP.inst._logged_message.emit(msg)
## Logs a message to the GodotAP log
...
func _poll() -> void: # Poll the websocket
	while true:
		...
		_socket_state = _socket.get_ready_state()
		match _socket_state:
			WebSocketPeer.STATE_CLOSED: # Exited; conclude the disconnection
				hang_clock.stop()
				if status != APStatus.DISCONNECTING:
					AP.log("Accidental disconnection!")
				status = APStatus.DISCONNECTED
				disconnected.emit()
	...
	if OS.is_debug_build():
		...
		cmd_manager.register_command(ConsoleCommand.new("/slot_data").debug()
			...
			.set_call(func(mgr: CommandManager, _cmd: ConsoleCommand, _msg: String):
				...
				mgr.console.add(folder)
				folder.add(BaseConsole.make_indented_block(JSON.stringify(AP.inst.conn.slot_data, "\t"), 25))
				folder.fold(false)
				))
...
static func get_item_class_color(flags: int) -> SpecialColor:
	if flags & ItemClassification.PROG:
		if AP.inst.AP_ENABLE_PROGUSEFUL and (flags & ItemClassification.USEFUL):
