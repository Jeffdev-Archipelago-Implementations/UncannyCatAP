...
static func from(json: Dictionary, recv: bool) -> NetworkItem:
	...
	v.src_player_id = json["player"] if recv else AP.inst.conn.player_id
	v.dest_player_id = AP.inst.conn.player_id if recv else json["player"]
	v.flags = json["flags"]
	return v
...
func get_name() -> String:
	return AP.inst.conn.get_gamedata_for_player(dest_player_id).get_item_name(id)
func _to_string():
	return "ITEM(%d at %d,player %d->%d,flags %d)" % [id,loc_id,src_player_id,dest_player_id,flags]
func output() -> ConsoleLabel:
	return BaseConsole.make_item(id, flags, AP.inst.conn.get_gamedata_for_player(dest_player_id))
