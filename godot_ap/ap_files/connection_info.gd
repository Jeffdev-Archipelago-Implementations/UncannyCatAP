...
func _load_locations() -> void:
	locations.clear()
	locs_by_name.clear()
	assert(not AP.inst._datapack_pending) # should never be called before datapacks are loaded...
	for locid in AP.inst.location_list():
...
func set_notify(key: String, proc: Callable) -> void:
	if not _notified_keys.has(key):
		AP.inst.send_command("SetNotify", {"keys": [key]})
...
func retrieve(key: String, proc: Callable) -> void:
	AP.inst.send_command("Get", {"keys": [key]})
...
func update_hint(loc: int, plyr: int, status: NetworkHint.Status) -> void:
	AP.inst.send_command("UpdateHint", {"location": loc, "player": plyr, "status": status})
...
func scout(location: int, create_as_hint: int, proc: Callable) -> void:
	...
	if create_as_hint or not item: # Always send if `create_as_hint`!
		AP.inst.send_command("LocationScouts", {"locations": [location], "create_as_hint": create_as_hint})
	if not proc: return
	if item:
		proc.call(item)
	else:
...
func force_scout_all() -> void: ## Scouts every location into the local cache
	AP.inst.send_command("LocationScouts", {"locations": slot_locations.keys(), "create_as_hint": 0})
...
func send_bounce(data: Dictionary, target_games: Array[String], target_slots: Array[String], target_tags: Array[String]) -> void:
	...
	if not cmd: return
	cmd.merge(data)
	AP.inst.send_command("Bounce", cmd)
...
func send_deathlink(cause: String = ""):
	...
	AP.inst.last_sent_deathlink_time = Time.get_unix_time_from_system()
	cmd["data"]["time"] = AP.inst.last_sent_deathlink_time
	send_bounce(cmd, [], [], [AP.inst.get_deathlink_tag()])
...
func send_traplink(trap_name: String):
	...
	cmd["data"]["source"] = get_player_name(-1, false)
	AP.inst.last_sent_traplink_time = Time.get_unix_time_from_system()
	cmd["data"]["time"] = AP.inst.last_sent_traplink_time
