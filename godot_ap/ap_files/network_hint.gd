...
func as_plain_string() -> String:
	return "%s %s '%s' (%s) for %s at '%s'" % [
		AP.inst.conn.get_player_name(item.src_player_id),
		"found" if status == Status.FOUND else "will find",
		item.get_name(), item.get_classification(),
		AP.inst.conn.get_player_name(item.dest_player_id),
		AP.inst.conn.get_gamedata_for_player(item.src_player_id).get_loc_name(item.loc_id)
	]
