...
static func make_player(id: int) -> ConsoleLabel:
	var player: NetworkPlayer = AP.inst.conn.get_player(id)
	var ttip = "Game: %s" % AP.inst.conn.get_slot(id).game
	if not player.alias.is_empty():
		ttip += "\nSlot: %s" % player.name
	var color := (AP.SpecialColor.OWN_PLAYER if id == AP.inst.conn.player_id else
...
func printjson_command(json: Dictionary) -> String:
	...
	match json.get("type"):
		"Chat":
			...
			var name_part: ConsoleLabel = AP.inst.conn.get_player(json["slot"]).output()
		...
		"Countdown":
			...
			output_data = true
		"ItemSend", "ItemCheat":
			if not AP.inst.AP_HIDE_NONLOCAL_ITEMSENDS:
				output_data = true
			elif int(json["receiving"]) == AP.inst.conn.player_id:
				output_data = true
			else:
				var ni := NetworkItem.from(json["item"], true)
				if ni.src_player_id == AP.inst.conn.player_id:
					output_data = true
		"Hint":
			if int(json["receiving"]) == AP.inst.conn.player_id:
				output_data = true
			else:
				var ni := NetworkItem.from(json["item"], true)
				if ni.src_player_id == AP.inst.conn.player_id:
		...
		"Join", "Part":
			...
			var plyr: NetworkPlayer = AP.inst.conn.get_player(json["slot"])
...
func printjson_out(elems: Array, flowbox: ConsoleHFlow) -> String:
	...
	for elem in elems:
		...
		match elem.get("type", "text"):
			...
			"player_id":
				var plyr_id = int(txt)
				part = AP.inst.conn.get_player(plyr_id).output()
			"item_id":
				var item_id = int(txt)
				var plyr_id = int(elem["player"])
				var data: DataCache = AP.inst.conn.get_gamedata_for_player(plyr_id)
...
static func printjson_out_str(elems: Array) -> String:
	...
	for elem in elems:
		...
		match elem.get("type", "text"):
			"player_id":
				var plyr_id = int(txt)
				txt = AP.inst.conn.get_player(plyr_id).output().text
			"item_id":
				var item_id = int(txt)
				var plyr_id = int(elem["player"])
				var data: DataCache = AP.inst.conn.get_gamedata_for_player(plyr_id)
