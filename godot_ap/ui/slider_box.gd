...
func _ready():
	...
	reset_size()
	AP.inst.connected.connect(func(_conn, _json):
		connect_btn.disabled = true
		disconnect_btn.disabled = false
		if AP.inst.AP_CONSOLE_CONNECTION_AUTO:
			slide_to(false))
	AP.inst.disconnected.connect(func():
		connect_btn.disabled = false
		disconnect_btn.disabled = true
		if AP.inst.AP_CONSOLE_CONNECTION_AUTO:
			slide_to(true))
	if AP.inst.AP_CONSOLE_CONNECTION_AUTO or AP.inst.AP_CONSOLE_CONNECTION_OPEN:
		is_open = true
