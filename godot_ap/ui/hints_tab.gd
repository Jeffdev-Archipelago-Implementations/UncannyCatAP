...
func sort_by_dest(a: NetworkHint, b: NetworkHint) -> int:
	return (AP.inst.conn.get_player_name(a.item.dest_player_id).nocasecmp_to(
		AP.inst.conn.get_player_name(b.item.dest_player_id)))
func sort_by_item(a: NetworkHint, b: NetworkHint) -> int:
	var a_data: DataCache = AP.inst.conn.get_gamedata_for_player(a.item.dest_player_id)
	var b_data: DataCache = AP.inst.conn.get_gamedata_for_player(b.item.dest_player_id)
...
func sort_by_src(a: NetworkHint, b: NetworkHint) -> int:
	return (AP.inst.conn.get_player_name(a.item.src_player_id).nocasecmp_to(
		AP.inst.conn.get_player_name(b.item.src_player_id)))
func sort_by_loc(a: NetworkHint, b: NetworkHint) -> int:
	var a_data: DataCache = AP.inst.conn.get_gamedata_for_player(a.item.src_player_id)
	var b_data: DataCache = AP.inst.conn.get_gamedata_for_player(b.item.src_player_id)
...
func reset_hints_to_empty() -> void:
	...
	load_hints([])
func _ready():
	AP.inst.connected.connect(func(conn: ConnectionInfo, _j: Dictionary):
		conn.set_hint_notify(self.load_hints))
	AP.inst.disconnected.connect(reset_hints_to_empty)
...
func refresh_hints():
	...
	for hint in _stored_hints:
		...
		if filter_allow(hint):
			var data: DataCache = AP.inst.conn.get_gamedata_for_player(hint.item.src_player_id)
			...
			if hint.item.dest_player_id == AP.inst.conn.player_id:
					...
					status.clicked.connect(func(_btn):
						...
						for s in [NetworkHint.Status.AVOID,NetworkHint.Status.NON_PRIORITY,NetworkHint.Status.PRIORITY]:
							...
							btn.pressed.connect(func():
								AP.inst.conn.update_hint(hint.item.loc_id, hint.item.src_player_id, s)
...
func filter_allow(hint: NetworkHint) -> bool:
	var recv_name := AP.inst.conn.get_player_name(hint.item.dest_player_id, false)
	var item_name := hint.item.get_name()
	var find_name := AP.inst.conn.get_player_name(hint.item.src_player_id, false)
	var loc_name := AP.inst.conn.get_gamedata_for_player(hint.item.src_player_id).get_loc_name(hint.item.loc_id)
